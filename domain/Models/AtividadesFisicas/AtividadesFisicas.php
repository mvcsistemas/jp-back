...
class AtividadesFisicas extends MVCModel
{
    use HasFactory, HasUuid;

    protected $table      = 'atividades_fisicas';
    ...
    public function filter(Builder $query, array $params = []): Builder
    {
        ...
        return $query
            ->when($uuid, function ($query) use ($uuid) {
                $query->where('atividades_fisicas.uuid', $uuid);
            })
            ->when($descricao, function ($query) use ($descricao) {
                $query->where('atividades_fisicas.descricao', $descricao);
            })
            ...
            ->when(!$tipo_ordenacao || !$campo_ordenacao, function ($query) {
                $query->orderBy('atividades_fisicas.descricao');
            });
    }
}
